...
class BaseGetUpMjxEnv(PipelineEnv):
    ...
    Suporta mapeamento de juntas, sensor data e vetorização em JAX.
    """
...
"""
        ...
        # Mapeamento de índices de juntas
...
class GetUpFrontMjxEnv(BaseGetUpMjxEnv):
    ...
    def reset(self, rng: jax.Array) -> State:
        ...
        # Posição inicial: de bruços (Front) [qpos[2]=0.25, rot=[0.707, 0, 0.707, 0]]
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        torso_pos = pipeline_state.x.pos[1]  # Torso pos z
        torso_height = torso_pos[2]
        
        torso_rot = pipeline_state.x.rot[1]
        torso_up = 1.0 - 2.0 * (torso_rot[1]**2 + torso_rot[2]**2)

        # 1. Recompensa de Altura
        ...
        # 2. Termos de Shaping Compartilhados
        ...
        # 3. Termos Específicos do FRONT (Tuck & Joelhos)
        ...
        # 4. Condição de Vitória (Standing Bonus)
        ...
        total_reward = height_reward + waist_penalty + symmetry_penalty + action_penalty + tuck_reward + step_penalty + standing_bonus
...
class GetUpBackMjxEnv(BaseGetUpMjxEnv):
    ...
    def reset(self, rng: jax.Array) -> State:
        qpos = self.sys.init_q
        # Posição inicial: com as costas no chão (Back) [qpos[2]=0.25, rot=[0.707, 0, -0.707, 0]]
        qpos = qpos.at[2].set(0.25)
        qpos = qpos.at[3].set(0.707)
        qpos = qpos.at[4].set(0.0)
        qpos = qpos.at[5].set(-0.707)
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        torso_pos = pipeline_state.x.pos[1]
        torso_height = torso_pos[2]
        ...
        # 2. Termos de Shaping Específicos do BACK
        waist_qpos = pipeline_state.qpos[self._waist_idx]
        waist_penalty = -10.0 * (waist_qpos ** 2)

        # Pés & Pernas (Tuck & Anti-espacate)
        l_knee = pipeline_state.qpos[self._l_knee_idx]
        r_knee = pipeline_state.qpos[self._r_knee_idx]
        ...
        total_reward = height_reward + waist_penalty + tuck_reward + leg_symmetry_penalty + action_penalty + step_penalty + standing_bonus
        done = jnp.where(torso_height < 0.05, 1.0, 0.0)
