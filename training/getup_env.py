...
class GetUpBackEnv(_MimicGetUpMixin, GetUpEnv):
    ...
    def _back_impulse_term(self, torso_height, torso_up):
        """Recompensa o IMPULSO (velocidade de subida) na transição chão -> em pé."""
        reward = 0.0
        # Só importa impulso enquanto o robô ainda não terminou de levantar
        if torso_height < 0.60:
            # 1. Velocidade vertical do centro do robô (root). qvel[2] = vel. linear em Z
            vertical_vel = self.data.qvel[2]
            reward += np.clip(vertical_vel, -0.5, 3.0) * 12.0
 
            # 2. Velocidade de EXTENSÃO das pernas (empurrão), não só o ângulo estático
            l_knee_dof = self.model.joint('Left_Knee_Pitch').dofadr[0]
            r_knee_dof = self.model.joint('Right_Knee_Pitch').dofadr[0]
            l_hip_dof = self.model.joint('Left_Hip_Pitch').dofadr[0]
            r_hip_dof = self.model.joint('Right_Hip_Pitch').dofadr[0]
 
            l_knee, r_knee = self.data.qpos[self.l_knee_idx], self.data.qpos[self.r_knee_idx]
            l_hip, r_hip = self.data.qpos[self.l_hip_pitch_idx], self.data.qpos[self.r_hip_pitch_idx]
 
            l_knee_vel = self.data.qvel[l_knee_dof]
            r_knee_vel = self.data.qvel[r_knee_dof]
            l_hip_vel = self.data.qvel[l_hip_dof]
            r_hip_vel = self.data.qvel[r_hip_dof]
 
            # extensão = ângulo indo de volta pra 0 (saindo do "tuck")
            l_knee_ext = -np.sign(l_knee) * l_knee_vel
            r_knee_ext = -np.sign(r_knee) * r_knee_vel
            l_hip_ext = -np.sign(l_hip) * l_hip_vel
            r_hip_ext = -np.sign(r_hip) * r_hip_vel
 
            reward += (max(0.0, l_knee_ext) + max(0.0, r_knee_ext)) * 4.0
            reward += (max(0.0, l_hip_ext) + max(0.0, r_hip_ext)) * 3.0
        return reward
    ...
    def compute_reward(self, action, obs):
        ...
        # 3.5. IMPULSO (NOVO) — recompensa velocidade de subida/extensão das pernas
        reward += self._back_impulse_term(torso_height, torso_up)
        ...
        # 2. Punição direta pela distância ENTRE os pés (mata o espacate)
