...
class Robot(ABC):
    ...
    def __init__(self, agent=None):
        """
        Creates a new robot linked to the given agent (optional).

        Args:
            agent: The main agent that owns this robot.
        """
        self.agent = agent
        self.server = getattr(agent, "server", None)
    ...
    def commit_motor_targets_pd(self) -> None:
        ...
        for motor_name, target_description in self.motor_targets.items():
            pos = target_description['target_position']
            kp = target_description['kp']
            kd = target_description['kd']
            motor_msg = f"({motor_name} {pos:.2f} 0.0 {kp:.2f} {kd:.2f} 0.0)"
            if self.server:
